...
export async function getServerSideProps({ query }) {
    const tutor_id = query.id
    const students = await getStudentsWithAllInformation();
...
export default function SearchStudents({ tutor_id, students }) {
    ...
    function formatResultsInfo(results) {
        let rows = []
        let index = 1

        for (const result of results) {
            rows.push(`${index}. ${result.module.module_name} (${result.module.module_code}): ${result.adjusted_marks == 0 ? result.marks + "% (The marks are not adjusted)" : result.adjusted_marks + "% (Original: " + result.marks + "%)"}`)
            ++index
        }
    ...
    function formatCommentsDetails(comments) {
        ...
        for (const comment of comments) {
            ...
            result.push(`${index}. ${senderName} commented "${comment.comment_data}" at ${formatDateTimeStringFromMs(parseInt(comment.created_time))}.`)
            ++index
        }
            ...
            setRows(filteredStudents.map(function (student) {
                return (
                    <Table.Row key={student.student_id}>
                        ...
                        <Table.Cell verticalAlign='top' style={{ whiteSpace: "pre-line" }}>{formatResultsInfo(student.results)}</Table.Cell>
